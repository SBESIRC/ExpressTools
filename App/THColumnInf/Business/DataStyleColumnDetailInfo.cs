...
namespace ThColumnInfo
{
    ...
    public class DataStyleColumnDetailInfo : ExtractBase
    {
        ...
        protected double searchBoundaryOffsetDis = 20.0;       
        ...
        {
            Point3d pt = originPt;
            TypedValue[] tvs = new TypedValue[] { new TypedValue((int)DxfCode.Start, "Line,Polyline,Arc") };
            SelectionFilter sf = new SelectionFilter(tvs);
            Point3d pt1 = originPt + new Vector3d(-1, -1, 0);
            Point3d pt3 = originPt + new Vector3d(1, 1, 0);
            PromptSelectionResult psr = ThColumnInfoUtils.SelectByRectangle(doc.Editor, pt1, pt3, PolygonSelectionMode.Crossing, sf);
            DBObjectCollection dbObjs= doc.Editor.TraceBoundary(originPt,false); //给的点要能选到边界
