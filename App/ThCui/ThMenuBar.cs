    ...
    public class ThMenuBar
    {
        ...
        public void CreateThMenu()
        {
            ...
            if (thMenu != null)//如果“我的菜单”还没有被添加，则添加菜单项
            {
                ...
                kps = new List<ThCommandInfo> {
                    new ThCommandInfo("插块断线 <THBBR>", "THBBR", "将选择的图块插入到直线/多段线时自动断线",true),
                    new ThCommandInfo("选块断线 <THBBE>", "THBBE", "点选单个图块，根据所需断线的切线方向自动调整图块角度且完成断线",true),
                    new ThCommandInfo("全选断线 <THBBS>", "THBBS", "批量选择需要断线的图块，根据各自所需断线的切线方向自动调整图块角度且完成断线",true)
                };
                kps.ForEach(kp =>
                {
                    menuModify.AddMenuItem(-1, kp.Name, id + kp.Command);
                ...
                kps = new List<ThCommandInfo> {
                    ...
                    new ThCommandInfo("版次信息修改 <THSVM>", "THSVM", "批量处理管线断线",true),
                    new ThCommandInfo("管线断线 <THLTR>", "THLTR", "批量修改图框内的版次信息或出图日期",true),
                    new ThCommandInfo("文字块镜像 <THMIR>", "THMIR", "镜像含文字块，使文字不反向",true)
                };
                kps.ForEach(kp =>
                {
                    menuModify.AddMenuItem(-1, kp.Name, id + kp.Command);
