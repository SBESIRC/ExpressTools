...
{
    public class VolumeReinforceRatioGRule : IRule
    {
        private VolumeReinforceRatioAModel vrra = null;
        private string rule = "（《高规》10.2.11-8）";
        public VolumeReinforceRatioGRule(VolumeReinforceRatioAModel vrra)
        {
            this.vrra = vrra;
        }
        ...
        public void Validate()
        {
            ...
            if(this.vrra.IsZHZ() && this.vrra.AntiSeismicGrade.Contains("非"))
            {
                if (calVolumnReinforceRatio < 0.008)
                {
                ...
                {
                    this.CorrectResults.Add("体积配箍率大于等于0.8% " + this.rule);
                }
            }
        }
        public List<string> GetCalculationSteps()
        {
            ...
            steps.Add("if (柱号[" + this.vrra.Code + "].Contains(\"ZHZ\") && 抗震等级[" + 
                this.vrra.AntiSeismicGrade +"].Contains(非)");
            steps.Add("  {");
            ...
            steps.Add("      else");
            steps.Add("       {");
            steps.Add("          Debugprint: 体积配箍率大于等于0.8% " + this.rule);
            steps.Add("       }");
            steps.Add("  }");
            steps.Add("");
            return steps;
