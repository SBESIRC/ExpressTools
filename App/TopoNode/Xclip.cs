...
namespace TopoNode
{
    ...
    {
        public BlockReference curBlock;
        public List<XClipInfo> curXclips;

        public ReferenceXclipInfo(BlockReference block, List<XClipInfo> clips = null)
        {
            curBlock = block;
            if (clips == null)
                        ...
                        if (fildict.Contains(spatialName))
                        {
                            ...
                            if (fil != null && fil.Definition.Enabled)
                            {
                                ...
                                foreach (Point2d pt in fil.Definition.GetPoints())
                                {
                                    ...
                                    tempPt = tempPt.TransformBy(fil.ClipSpaceToWorldCoordinateSystemTransform); //从Ucs到父级坐标点
                                ...
                                xClipInfo.BlockTransForm = blockRef.BlockTransform;
                            }
                        }
                    }
                }
