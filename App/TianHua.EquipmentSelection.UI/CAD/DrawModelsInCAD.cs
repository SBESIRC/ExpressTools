...
namespace TianHua.FanSelection.UI.CAD
{
    public class DrawHtfcModelsInCAD : DrawModels
    {
        public DrawHtfcModelsInCAD(string jsonpath)
        {
            ...
            HighModels = ListFanParameters.ToGeometries(comparer, "低").Cast<ILineString>().ToList();
            if (jsonpath.Contains(ThFanSelectionCommon.HTFC_Parameters_Double))
            {
                LowModels = ListFanParameters.ToGeometries(comparer, "高").Cast<ILineString>().ToList();
            }
    ...
    public class DrawAxialModelsInCAD : DrawModels
    {
        public DrawAxialModelsInCAD(string jsonpath)
        {
            ...
            HighModels = ListAxialFanParameters.ToGeometries(comparer, "低").Cast<ILineString>().ToList();
            if (jsonpath.Contains(ThFanSelectionCommon.AXIAL_Parameters_Double))
            {
                LowModels = ListAxialFanParameters.ToGeometries(comparer, "高").Cast<ILineString>().ToList();
            }
        }
    ...
    {
        public List<ILineString> HighModels { get; set; }
        public List<ILineString> LowModels { get; set; }
